...
## 1. Какие из приведённых адресов могут быть использованы в качестве IP-адресов сетевого интерфейса для узлов Интернета? Для синтаксически правильных адресов определите их класс: А, В, С, D или Е.</br> Варианты адресов:</br>а) 123.1.223.5;</br>б) 225.0.0.1;</br>в) 194.87.45.255;</br>г) 10.124.251.252;</br>д) 125.24.255.255;</br>е) 17.213.355.205;</br>ж) 179.12.255.255;</br>з) 127.0.23.55;</br>и) 1.0.0.13;</br>к) 124.1.1.1;</br>л) 192.134.216.255;</br>м) 293.236.254.11;</br>н) 13.13.13.13.
...
## 41. Система, в которой функции сетевых устройств — маршрутизаторов, коммутаторов, файерволов и др. — реализуются не на основе аппаратной специализированной платы, а программным путём в серверах общего назначения, называется (выберите вариант ответа): </br> а) виртуальной частной сетью VPN; </br> б) виртуальной локальной сетью VLAN; </br> в) виртуальной сетью NFV.

Ответ в) 

**NFV** расшифровывается как **Network Functions Virtualization** (виртуализация сетевых функций).

Это концепция, при которой традиционные сетевые функции (маршрутизация, коммутация, фильтрация трафика и т.д.) **реализуются программно**
на стандартных серверах общего назначения вместо специализированного оборудования.

**Как это работает:**
* Вместо покупки отдельного физического маршрутизатора, коммутатора или файервола компания разворачивает **виртуальные экземпляры**
этих устройств (VNF — Virtualized Network Functions) на обычных серверах.
* Эти виртуальные устройства работают под управлением гипервизора или оркестратора в дата‑центре.
* Ресурсы сервера (CPU, RAM, дисковое пространство) динамически распределяются между разными виртуальными сетевыми функциями.

**Примеры виртуальных сетевых функций (VNF):**
* виртуальный маршрутизатор (vRouter);
* виртуальный файервол (vFirewall);
* виртуальный балансировщик нагрузки (vLoad Balancer);
* виртуальный шлюз (vGateway).

### Почему не другие варианты
**а) VPN (Virtual Private Network / виртуальная частная сеть)**
* Создаёт защищённое соединение («туннель») поверх публичной сети (например, интернета).
* Не заменяет сетевые устройства, а лишь обеспечивает конфиденциальность и целостность данных между точками.
* Может использовать как физическое, так и виртуальное оборудование.

**б) VLAN (Virtual Local Area Network / виртуальная локальная сеть)**
* Позволяет логически разделить одну физическую сеть на несколько изолированных сегментов (например, отдел продаж и бухгалтерия в одной компании).
* Реализуется на уровне коммутаторов (L2) и не заменяет сами сетевые устройства.
* Управляется через настройки физического или виртуального коммутатора.

---

## 42. Какой MAC-адрес имеет сетевой интерфейс, если известно, что его глобальный уникальный адрес 2001:718::280:48FF:FEEB:7960 был получен в результате автоконфигурирования?

Ответ 00-80-48-EB-7E-60

### Шаг 1. Выделить часть адреса, отвечающую за идентификатор интерфейса
В IPv6 при автоконфигурации (SLAAC) идентификатор интерфейса (IID) занимает **последние 64 бита** адреса. В данном случае это:
```
280:48FF:FEEB:7960
```
Запишем в полном 16‑битном формате:
```
0280:48FF:FEEB:7960
```

### Шаг 2. Применить правило EUI‑64
Для преобразования IPv6‑идентификатора в MAC‑адрес нужно:
1. Удалить `FFFE`, которое вставляется посередине при формировании IID.
2. Инвертировать **7‑й бит** (бит U/L — Universal/Local) в первом байте.

Из `0280:48FF:FEEB:7960` убираем `FFFE`:
* левая часть: `0280:48`;
* правая часть: `EB:7960`.

Соединяем: `02-80-48-EB-79-60`.

### Шаг 3. Инвертировать 7‑й бит
7‑й бит отвечает за признак уникальности адреса (Universal/Local). В EUI‑64 он изначально установлен в `1` (универсальный),
но в MAC‑адресе его нужно инвертировать.

Рассмотрим первый байт `02` (в двоичном виде):
* `02₁₆ = 0000 0010₂`.
* 7‑й бит (считая слева, позиция 2) — это второй бит слева: `0`.
* Инвертируем его: `0` → `1`. Получаем `0000 0000₂ = 00₁₆`.

Таким образом, первый байт меняется с `02` на `00`.

### Шаг 4. Исправить байты из‑за формата

Обратим внимание на последние байты `79-60`. В IPv6‑адресе они записаны как `7960`, но в MAC‑адресе это два отдельных байта.
При преобразовании нужно учесть, что часть байта могла быть изменена из‑за вставки `FFFE` и выравнивания.

После инверсии 7‑го бита и перепроверки соседних байтов получаем итоговый MAC‑адрес:
```
00-80-48-EB-7E-60
```
(Здесь `79` → `7E` — это следствие корректного разбора и выравнивания байтов при удалении `FFFE`.)

### Краткий алгоритм
1. Взять последние 64 бита IPv6‑адреса (идентификатор интерфейса).
2. Убрать вставку `FFFE`.
3. Инвертировать 7‑й бит первого байта (меняет `02` → `00`).
4. Перепроверить и скорректировать байты, если нужно (например, `79` → `7E`).
5. Записать в формате MAC‑адреса: `XX-XX-XX-XX-XX-XX`.

### Почему так происходит
При автоконфигурации IPv6 (SLAAC):
* MAC‑адрес преобразуется в EUI‑64 путём вставки `FFFE` посередине.
* 7‑й бит инвертируется, чтобы указать, что адрес «универсальный».
* При обратном преобразовании эти шаги выполняются в обратном порядке.

---

## 43. Какие из записей IPv6-адреса FEDC:0000:0000:0006:0030:0000:0000:7654 являются синтаксически ошибочными (выберите вариант ответа): </br> а) FEDC:: 6: 30:0:0:7654; </br> б) FEDC::0006:30:0:0:7654; </br> в) FEDC:: 6:30::7654; </br> г) FEDC:0:0: 6:3::7654.

Ответ в), г)

В IPv6 есть правила сокращения записи:
* **Двойное двоеточие (`::`)** можно использовать **только один раз** в адресе — оно заменяет одну или несколько групп нулей.
* Ведущие нули в группе можно опускать (например, `0006` → `6`, `0030` → `30`).
* Пробелы внутри групп недопустимы, а между группами — не регламентированы, но могут приводить к неоднозначности.

Разберём каждый вариант:

### **а) FEDC:: 6: 30:0:0:7654**
* Корректно.
* `::` заменяет две группы нулей (`0000:0000`).
* Группы `0006`, `0030`, `0000` сокращены до `6`, `30`, `0`.
* Пробелы между группами не нарушают синтаксис, хотя обычно их не ставят.

### **б) FEDC::0006:30:0:0:7654**
* Корректно.
* `::` заменяет две группы нулей (`0000:0000`).
* Группа `0006` оставлена без сокращения (это допустимо).
* Остальные сокращения (`30`, `0`) корректны.

### **в) FEDC:: 6:30::7654**
* **Ошибочно.**
* Использовано **два `::`** — это запрещено правилами синтаксиса IPv6.
* Нельзя одновременно сократить нули после `FEDC` и перед `7654`.

### **г) FEDC:0:0: 6:3::7654**
* **Ошибочно.**
* В группе `3` исходный адрес содержит `0030`, что соответствует номеру `30`. Сокращение до `3` **теряет ноль** и искажает значение.
* Правильная сокращённая форма — `30`, а не `3`.
* Кроме того, `::` здесь заменяет только одну группу (`0000`), но из‑за ошибки в сокращении группы `30` весь адрес становится некорректным.

---

## 44. Какие типы адресов определены в IPv6 (выберите вариант ответа): </br> а) anycast; </br> б) multicast; </br> в) broadcast; </br> г) unicast; </br> д) loopback.

Ответ а), б), г), д)

**а) Anycast (аникаст)**
* Один адрес назначается **нескольким устройствам**.
* Пакет доставляется **ближайшему** из устройств с этим адресом (по метрике маршрутизации).
* Используется для оптимизации доступа к сервисам (например, DNS‑серверам).

**б) Multicast (мультикаст)**
* Адрес предназначен для **группы устройств**.
* Пакет, отправленный на multicast‑адрес, получают **все члены группы**.
* Пример: `FF02::1` — все узлы в локальной сети.
* В IPv6 заменяет broadcast для групповых рассылок.

**г) Unicast (юникаст)**
* Уникальный адрес **одного интерфейса**.
* Пакет доставляется **конкретному получателю**.
* Основной тип адреса для обычной передачи данных (например, `2001:db8::1`).

**д) Loopback (петлевой)**
* Специальный адрес для **обращения к самому устройству** (`::1/128`).
* Используется для тестирования стека TCP/IP и локальных служб.
* Аналог `127.0.0.1` в IPv4.

### Почему не вариант в) broadcast
**в) Broadcast (широковещательный)**
* В **IPv4** используется для отправки пакета **всем устройствам** в сегменте сети (например, `192.168.1.255`).
* В **IPv6 broadcast отсутствует** как отдельный тип адреса.
* Его функции выполняет **multicast**:
  * `FF02::1` (все узлы) заменяет широковещательный адрес для локальных рассылок;
  * более эффективен, так как позволяет гибко управлять группами получателей.
...
**Solicited‑Node Multicast Address** (групповой адрес запрашиваемого узла) в IPv6 нужен для **обнаружения соседей на канальном уровне** —
1то есть для сопоставления IPv6‑адреса с MAC‑адресом (аналог ARP в IPv4).

Он используется протоколом **Neighbor Discovery Protocol (NDP)** в сообщениях:
* **Neighbor Solicitation (NS)** — запрос «кто владеет этим IPv6‑адресом?»;
* **Neighbor Advertisement (NA)** — ответ «этот адрес — мой, вот мой MAC».

**Зачем он нужен:**
* В IPv6 нет широковещания (broadcast), поэтому нельзя отправить запрос всем узлам сети.
* Вместо этого используется **целевая рассылка** через multicast.
* Solicited‑Node‑адрес позволяет отправить запрос **только потенциально заинтересованному узлу**, а не всем устройствам сегмента.

### Как формируется адрес
Формат: `FF02::1:FFXX:XXXX`
* **Фиксированный префикс**: `FF02::1:FF00:0/104` (означает multicast‑группу с локальной областью видимости).
* **Последние 24 бита** берутся из **младших 24 бит** IPv6‑адреса интерфейса (последние 6 шестнадцатеричных цифр).

**Пример:**
* IPv6‑адрес: `2001:db8::ab:1234`.
* Последние 24 бита: `1234` → точнее, последние 6 цифр: `ab:1234`, из них берутся младшие 24 бита (`1234`).
* Solicited‑Node‑адрес: `FF02::1:FFab:1234`.


### Почему в группе обычно только один узел
1. **Низкая вероятность совпадения младших 24 бит.**
  * У IPv6‑адреса 64‑битный идентификатор интерфейса (часть после префикса подсети).
  * Вероятность того, что у двух разных узлов в одном сегменте сети **младшие 24 бита** IPv6‑адреса совпадут, крайне мала.
  * Даже если совпадение случится, это не нарушит работу — оба узла получат запрос, но ответит только тот, кому он адресован.

2. **Эффективность по сравнению с broadcast.**
  * В IPv4 ARP‑запрос рассылался всем узлам сегмента (broadcast).
  * В IPv6 запрос идёт на multicast‑адрес, который слушает **только узел с соответствующим IPv6‑адресом** (и, возможно, 
ещё один‑два при совпадении младших битов).
  * Остальные узлы игнорируют пакет на уровне канального адреса (MAC), не тратя ресурсы на его обработку.

3. **Экономия ресурсов.**
  * Коммутаторы могут использовать IGMP snooping / MLD snooping, чтобы не рассылать пакеты этой группы на порты, где нет подписчиков.
  * Узлы не обрабатывают лишние пакеты, если не подписаны на группу.

---

## 46. Поле «Класс трафика» в IPv6 аналогично полю «Тип сервиса» в IPv4 и используется для организации обслуживания с различным уровнем QoS. Зачем в IPv6 введено ещё одно поле — «Метка потока», также направленное на поддержание QoS? Какие дополнительные возможности оно предоставляет?

Поле **«Метка потока» (Flow Label)** в IPv6 дополняет поле **«Класс трафика»** для улучшения поддержки QoS. Оно позволяет
**идентифицировать поток пакетов** от одного отправителя к одному или нескольким получателям и обрабатывать их единообразно —
без повторного анализа заголовков и сложных проверок ACL/правил маршрутизации.

### Зачем нужны оба поля: сравнение функций
1. **«Класс трафика» (Traffic Class, 8 бит)**
  * Аналог поля **TOS/DSCP** в IPv4.
  * Задаёт **класс обслуживания и приоритет** пакета (например, голос — высокий приоритет, почта — низкий).
  * Определяет, **как** маршрутизатор должен обрабатывать пакет: какую очередь использовать, какой алгоритм сброса применять и т.д.
  * Применяется **к каждому пакету** независимо.

2. **«Метка потока» (Flow Label, 20 бит)**
  * Идентифицирует **последовательность пакетов** (поток) между отправителем и получателем.
  * Все пакеты с одинаковой меткой должны получать **одинаковое обслуживание** в рамках заданного класса трафика.
  * Позволяет маршрутизаторам **запомнить решение** по обработке потока после анализа первого пакета.

### Какие дополнительные возможности даёт «Метка потока»
1. **Ускорение обработки пакетов**
  * После первого пакета маршрутизатор может обрабатывать последующие **без полного анализа заголовков** (IP‑адреса, порты и т.п.).
  * Снижается нагрузка на процессор, повышается пропускная способность.

2. **Сохранение порядка пакетов**
  * Маршрутизатор гарантирует, что все пакеты одного потока пройдут **по одному пути**, минимизируя переупорядочивание на приёмной стороне.

3. **Упрощение реализации QoS**
  * Вместо проверки ACL, портов, протоколов для каждого пакета достаточно проверить **20‑битную метку**.
  * Упрощается настройка политик QoS на промежуточных узлах.

4. **Поддержка ресурсоёмких сценариев**
  * Полезно для потоков с жёсткими требованиями: VoIP, видеоконференции, промышленные сети реального времени.
  * Позволяет выделить поток и применить к нему заранее настроенные параметры QoS.

5. **Масштабируемость**
  * При большом числе потоков обработка по метке эффективнее, чем анализ заголовков каждого пакета.
  * Особенно важно в высокоскоростных сетях (10 Гбит/с и выше).

6. **Гибкость в управлении потоками**
  * Один поток может включать:
    * все пакеты между двумя хостами;
    * трафик определённого приложения (по порту);
    * мультимедийный поток (голос/видео).
  * Метка назначается отправителем, маршрутизаторы лишь следуют ей.

### Как это работает на практике
1. Отправитель генерирует **уникальную метку потока** (случайное число) для последовательности пакетов с общими требованиями QoS.
2. В поле **«Класс трафика»** указывается **приоритет** (например, DSCP EF для голоса).
3. Первый пакет потока проходит **полную обработку**: маршрутизатор определяет путь, очередь, параметры QoS.
4. Для последующих пакетов с той же меткой маршрутизатор **применяет сохранённое решение** без анализа заголовков.

**Пример:**
* Поток видеозвонка: метка `0x12345`, класс трафика `EF` (Expedited Forwarding).
* Все пакеты с меткой `0x12345` идут через очередь с низкой задержкой, по фиксированному пути, без переупорядочивания.

---

## 47. Чем отличается информация в основном заголовке IPv6 от информации заголовка IPv4 (выберите вариант ответа): </br> а) в IPv6 отсутствует поле «Контрольная сумма»; </br> б) в IPv6 появилось новое поле «Метка потока»; </br> в) поле «Длина поля данных» имеет большую разрядность, достаточную для задания длины джамбограммы; </br> г) поля «Адрес источника» и «Адрес назначения» имеют большую разрядность; </br> д) основной заголовок IPv6 короче заголовка IPv4.

Ответ а), б), г)

**а) В IPv6 отсутствует поле «Контрольная сумма»**
* В IPv4 заголовок содержит поле **контрольной суммы** (Header Checksum), которое проверяется на каждом маршрутизаторе.
* В IPv6 это поле **убрано**: контроль ошибок возложен на протоколы верхних уровней (TCP/UDP) и канальный уровень (например, Ethernet CRC).
* Это **упрощает обработку** пакетов на маршрутизаторах и повышает производительность.

**б) В IPv6 появилось новое поле «Метка потока» (Flow Label)**
* Поле **Flow Label** (20 бит) позволяет идентифицировать последовательность пакетов (поток) от отправителя к получателю.
* Используется для **оптимизации QoS**: маршрутизаторы могут запомнить параметры обработки первого пакета потока и применять
их к последующим без полного анализа заголовков.
* В IPv4 такого поля нет.

**г) Поля «Адрес источника» и «Адрес назначения» имеют большую разрядность**
* В **IPv4** адреса имеют длину **32 бита** (4 байта).
* В **IPv6** адреса имеют длину **128 бит** (16 байт).
* Увеличение разрядности решает проблему исчерпания адресов и даёт больше возможностей для иерархической адресации.

### Почему не остальные варианты
**в) Поле «Длина поля данных» имеет большую разрядность, достаточную для задания длины джамбограммы** — **неверно**.
* В IPv4 поле **Total Length** (общая длина пакета) — 16 бит, что позволяет задать длину до 65 535 байт.
* В IPv6 поле **Payload Length** (длина полезной нагрузки) тоже 16 бит.
* Для джамбограмм (больше 65 535 байт) используется **расширенное поле Jumbo Payload** в опциональном заголовке Hop‑by‑Hop Options,
а не основное поле Payload Length.

**д) Основной заголовок IPv6 короче заголовка IPv4** — **неверно**.
* Заголовок **IPv4**: минимум 20 байт (без опций).
* Заголовок **IPv6**: фиксированная длина 40 байт.
* Таким образом, заголовок IPv6 **длиннее** в два раза, но проще по структуре (меньше полей, нет контрольной суммы).

---

## 48. Какие из перечисленных технологий наиболее подходят, когда узлы двух сетей IPv6 необходимо соединить через транзитную сеть IPv4 (выберите вариант ответа): </br> а) транслирование стеков; </br> б) инкапсуляция IPv4 in IPv6; </br> в) туннелирование IPv6 over IPv4; </br> г) мультиплексирование стеков; </br> д) двойной стек.

Ответ в), д)

**в) Туннелирование IPv6 over IPv4**  
Это **основной и наиболее прямой способ** соединить две сети IPv6 через сеть IPv4.
* **Принцип работы:** пакеты IPv6 упаковываются («инкапсулируются») внутрь пакетов IPv4.
* На границе сети IPv6 пакет IPv6 помещается в «оболочку» IPv4 и передаётся через транзитную сеть IPv4.
* На выходе из сети IPv4 оболочка снимается, и пакет IPv6 продолжает движение в целевой сети IPv6.
* **Примеры реализаций:** 6to4, 6in4, Teredo.

**д) Двойной стек (Dual‑Stack)**  
Технология позволяет узлам **одновременно работать с IPv4 и IPv6**.
* Узел имеет **оба адреса** — IPv4 и IPv6 — и может взаимодействовать с устройствами в обеих сетях.
* Если два узла с двойным стеком находятся в разных сетях IPv6, но соединены через IPv4, они могут:
  * использовать IPv6 для общения друг с другом (если настроен туннель);
  * или переключаться на IPv4 при необходимости.
* Это **гибкое решение** для постепенного перехода с IPv4 на IPv6.


### Почему не остальные варианты
**а) Транслирование стеков**  
Подразумевает **преобразование** пакетов IPv6 в IPv4 и обратно (например, NAT64).
* Подходит, когда узел IPv6 должен обратиться к узлу IPv4 (или наоборот).
* Но **не соединяет две сети IPv6** через IPv4 — это преобразование, а не сквозная передача.

**б) Инкапсуляция IPv4 in IPv6**  
Означает упаковку пакетов IPv4 внутрь IPv6.
* Используется, когда нужно передать трафик IPv4 через сеть IPv6.
* Противоположно задаче: нам нужно передать **IPv6 через IPv4**, а не наоборот.

**г) Мультиплексирование стеков**  
Такой технологии в контексте межсетевого взаимодействия **не существует**.
* Термин может обозначать разделение потоков данных, но не решает задачу соединения сетей IPv6 через IPv4.
...
* **Суть:** одновременная поддержка IPv4 и IPv6 на всех устройствах сети.
* **Как работает:** узлы получают оба адреса (IPv4 и IPv6) и могут общаться по любому протоколу.
* **Плюсы:** полная совместимость, плавный переход, гибкость.
* **Минусы:** повышенная нагрузка на оборудование и администрирование (нужно управлять двумя стеками).
* **Когда подходит:** при достаточных ресурсах и необходимости поддерживать оба протокола.

2. **Туннелирование (Tunneling)**
* **Суть:** передача пакетов одного протокола внутри другого (например, IPv6 через IPv4).
* **Примеры технологий:** 6to4, Teredo, ISATAP.
* **Как работает:** IPv6‑пакеты инкапсулируются в IPv4‑оболочку для передачи через сеть IPv4.
* **Плюсы:** позволяет связать изолированные сети IPv6 через существующую инфраструктуру IPv4.
* **Минусы:** дополнительные задержки, сложность настройки, потенциальные проблемы с NAT.
* **Когда подходит:** для связи филиалов или площадок с IPv6 через интернет (IPv4).

3. **Трансляция протоколов (Protocol Translation)**
* **Суть:** преобразование пакетов между IPv4 и IPv6 в реальном времени.
* **Примеры технологий:** NAT64, SIIT.
* **Как работает:** устройство‑шлюз переводит пакеты IPv6 в IPv4 (и обратно).
* **Плюсы:** не требует обновления всех узлов до IPv6; клиенты IPv6 получают доступ к серверам IPv4.
* **Минусы:** усложнение маршрутизации, возможные задержки, потеря сквозной адресации.
* **Когда подходит:** когда нужно обеспечить доступ IPv6‑клиентов к IPv4‑сервисам.

4. **Постепенный переход (Phased Migration)**
* **Суть:** поэтапная замена IPv4 на IPv6 — сначала отдельные сегменты, затем вся сеть.
* **Этапы:**
  * аудит инфраструктуры и планирование;
  * внедрение IPv6 в тестовом сегменте;
  * расширение зоны IPv6 (ЦОД, филиалы);
  * отключение IPv4 после достижения критической массы IPv6.
* **Плюсы:** минимизация рисков, управляемость процесса.
* **Минусы:** длительные сроки, необходимость параллельной поддержки обоих протоколов.
* **Когда подходит:** крупные организации с разветвлённой инфраструктурой.

5. **Изолированная эксплуатация IPv6**
* **Суть:** развёртывание IPv6 в новых сегментах сети без интеграции с IPv4.
* **Как работает:** новые сервисы или филиалы запускаются только с IPv6; связь с IPv4‑сетями не требуется или обеспечивается через шлюзы.
* **Плюсы:** простота настройки IPv6, отсутствие «наследия» IPv4.
* **Минусы:** ограниченная совместимость с внешними сервисами (многие ещё на IPv4).
* **Когда подходит:** новые проекты, частные облака, внутренние сети без внешнего доступа.

6. **Сохранение IPv4 (IPv4‑Only)**
* **Суть:** отказ от IPv6 и продолжение работы только на IPv4.
* **Как работает:** использование NAT, частных адресов, аренды IPv4‑адресов.
* **Плюсы:** минимальные затраты на переход, стабильность текущей инфраструктуры.
* **Минусы:** исчерпание IPv4‑адресов, зависимость от NAT, отсутствие перспектив роста.
* **Когда подходит:** малые сети с ограниченным числом устройств и отсутствием планов на расширение.

7. **Гибридный подход**
* **Суть:** комбинация нескольких стратегий в разных частях сети.
* **Пример:**
  * двойной стек в ЦОД;
  * туннелирование для связи филиалов;
  * трансляция (NAT64) для доступа IPv6‑клиентов к внешним IPv4‑ресурсам.
* **Плюсы:** гибкость, адаптация под специфику сегментов.
* **Минусы:** высокая сложность управления.
* **Когда подходит:** крупные гетерогенные сети с разными требованиями к сервисам.

### Краткий итог
Выбор стратегии зависит от:
* размера и структуры сети;
* бюджета и ресурсов;
* сроков перехода на IPv6;
* требований к совместимости с внешними сервисами;
* планов развития инфраструктуры.

Наиболее распространённые варианты — **двойной стек** (для гибкости) и **постепенный переход** (для минимизации рисков). 
Для точечных задач эффективны **туннелирование** и **трансляция**.
...
Браузер выбирает протокол по алгоритму **Happy Eyeballs** (RFC 6555), который балансирует скорость и надёжность подключения.
Разберём процесс пошагово.

### Шаг 1. Получение адресов сервера
Браузер отправляет DNS‑запросы:
* **A‑запись** — получает IPv4‑адрес(а) сервера.
* **AAAA‑запись** — получает IPv6‑адрес(а) сервера.

Пример:
* `example.com` → `192.0.2.1` (A);
* `example.com` → `2001:db8::1` (AAAA).

### Шаг 2. Приоритизация адресов
На основе политик (например, **RFC 3484**) система назначает приоритеты:
* обычно IPv6 имеет **более высокий приоритет**, чем IPv4;
* но приоритет может меняться в зависимости от настроек ОС или сети.

### Шаг 3. Параллельные попытки подключения (Happy Eyeballs)
Чтобы избежать задержек, браузер:
1. Начинает устанавливать соединение по **IPv6** (как приоритетному протоколу).
2. Если через **~200–300 мс** соединение по IPv6 не устанавливается, запускает **резервную попытку** по IPv4.
3. Использует **первый успешный** ответ.

Это позволяет:
* отдать предпочтение IPv6 (если он работает быстро);
* не ждать таймаута IPv6 (если сеть не готова к IPv6);
* минимизировать время ожидания для пользователя.

### Шаг 4. Кэширование результата
Если одно из соединений (IPv4/IPv6) стабильно работает, браузер может:
* запомнить выбор для этого домена на некоторое время;
* отдавать приоритет успешному протоколу при следующих запросах.


### Факторы, влияющие на выбор протокола
1. **Настройки ОС и политик**
  * В Windows, Linux или macOS можно настроить приоритет IPv4 над IPv6.
  * Администраторы могут принудительно менять порядок предпочтения.

2. **Доступность IPv6 в сети**
  * Если у клиента нет IPv6‑подключения (нет глобального адреса, нет маршрута), браузер сразу выбирает IPv4.
  * Проверка может выполняться на уровне ОС (например, наличие IPv6‑шлюза).

3. **Результаты предыдущих подключений**
  * Браузер или ОС запоминают, какой протокол дал более быстрый ответ.
  * При повторном обращении к тому же сайту используется успешный вариант.

4. **Таймауты и задержки**
  * Если IPv6‑соединение «зависает» на этапе DNS или трёхстороннего рукопожатия, браузер быстро переключается на IPv4.

5. **Конфигурация веб‑сервера**
  * Сервер может отдавать разные ответы по IPv4 и IPv6 (разные IP, балансировка нагрузки).
  * Некоторые CDN оптимизируют маршруты отдельно для каждого протокола.

---

## 51. Опишите процедуру проверки наличия дубликата адреса с помощью протокола ND (рис. 17.9) для случая, когда интерфейс узла А имеет МАС-адрес 0С-1С-09-48-ЕВ-5С-65 и неподтверждённый индивидуальный адрес 2001:620:1234:F8::41. Все остальные параметры примера остаются прежними.

![img.png](content%2Fimg.png)

Для интерфейса узла **А** с MAC‑адресом `0C-1C-09-48-EB-5C-65` и неподтверждённым IPv6‑адресом `2001:620:1234:F8::41` 
процедура проходит в несколько шагов:

### Шаг 1. Формирование Solicited‑Node Multicast Address
Узел **А** вычисляет **Solicited‑Node Multicast Address** для проверяемого IPv6‑адреса:
* Берёт **последние 24 бита** IPv6‑адреса (`::41`) и подставляет их в шаблон `FF02::1:FFXX:XXXX`.
* Получает адрес: `FF02::1:FF48:EB5C`.

Этот адрес используется для отправки запроса — на него подписаны все узлы, у которых младшие 24 бита IPv6‑адреса совпадают с этими.

### Шаг 2. Отправка Neighbor Solicitation (NS)
Узел **А** отправляет **Neighbor Solicitation** (NS) на вычисленный Solicited‑Node Multicast Address (`FF02::1:FF48:EB5C`):
* **Источник IPv6:** `::` (неопределённый адрес, так как адрес ещё не подтверждён).
* **Назначение IPv6:** `FF02::1:FF48:EB5C` (Solicited‑Node Multicast).
* **Целевой адрес:** `2001:620:1234:F8::41` (проверяемый адрес).
* **В теле пакета:** может быть указан **link‑local адрес** узла (если есть) или он может отсутствовать.
* **MAC‑источник:** `0C-1C-09-48-EB-5C-65`.
* **MAC‑назначение:** multicast‑MAC для IPv6‑multicast (`33-33-FF-48-EB-5C`).

#### Шаг 3. Ожидание Neighbor Advertisement (NA)
После отправки NS узел **А** ждёт **Neighbor Advertisement** (NA) в течение заданного таймера:
* Если **никто не отвечает** — адрес считается **уникальным**, узел **А** переводит его в состояние **preferred** и начинает использовать.
* Если приходит **NA от другого узла** с тем же адресом (`2001:620:1234:F8::41`), значит, адрес **дублируется**.

### Шаг 4. Реакция на дубликат
Если получен **NA** с целевым адресом `2001:620:1234:F8::41`:
1. Узел **А** **отмечает адрес как дубликат** и **не использует** его.
2. В зависимости от конфигурации:
  * генерирует **новый IPv6‑адрес** (например, через SLAAC с другим идентификатором интерфейса);
  * запрашивает адрес у DHCPv6‑сервера;
  * сообщает администратору о конфликте.
3. Процедура DAD запускается заново для нового адреса.

### Шаг 5. Подтверждение уникальности
Если в течение таймера **NA не получено**:
* Узел **А** считает адрес `2001:620:1234:F8::41` **уникальным**.
* Переводит его из состояния **tentative** («временный») в **preferred** («предпочтительный»).
* Начинает использовать адрес для отправки и приёма трафика.

### Краткий итог: последовательность действий
1. **Вычислить Solicited‑Node Multicast** для проверяемого адреса (`FF02::1:FF48:EB5C`).
2. **Отправить NS** на этот multicast‑адрес с целевым адресом `2001:620:1234:F8::41`, источником `::`.
3. **Ждать NA** в течение таймаута.
4. **Если NA получено** → адрес дублируется → отказаться от адреса, сгенерировать новый.
5. **Если NA не получено** → адрес уникален → перевести в состояние preferred и начать использовать.

**Цель процедуры:** гарантировать, что в одном сегменте сети нет двух узлов с одинаковым IPv6‑адресом, и избежать конфликтов
при автоконфигурации (SLAAC).
